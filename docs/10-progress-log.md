...
- 문서 뷰어 UX/렌더링 고도화 (검색 + 코드 하이라이트 + YAML 개선)
  ...
  - 검색 입력 UX 개선: 검색어 입력 시 검색 결과 목록을 표시하고, 빈 값이면 기본 트리 탐색으로 자동 복귀
  - 검색 DOM/CSS 정리: `search-wrap`, `search-results`, `search-item-path`, `search-item-snippet` 구조로 분리하여 트리와 검색결과 렌더 영역 분리
  - 상단 메뉴 토글 추가: `메뉴 숨기기` 클릭 시 사이드바 닫기, `메뉴 보이기` 클릭 시 다시 열기, 상태를 로컬 스토리지에 저장
