...
const MONSTER_MAIN_SPRITE_Y = -96 + UNIT_SPRITE_Y_SHIFT;
const MONSTER_SUPPORT1_Y = -78 + UNIT_SPRITE_Y_SHIFT;
...
export class MainScene extends Phaser.Scene {
  ...
  private heroBattleAnimKey = '';
  private heroDeathAnimKey = '';
  private companionSprites: Phaser.GameObjects.Sprite[] = [];
  private companionBattleAnimKeys: string[] = [];
  private monsterSprite!: Phaser.GameObjects.Sprite;
  private monsterBattleAnimKey = '';
  private monsterDeathAnimKey = '';
  private monsterBaseFrame = -1;
  ...
  private createCompanionAvatars(width: number, height: number): void {
    ...
    active.forEach((companion, idx) => {
      ...
      this.companionBattleAnimKeys.push(anim.battleKey);
  ...
  private playHeroBattleAnimation(): void {
    this.playLoopAnimationIfNeeded(this.heroSprite, this.heroBattleAnimKey);
    this.playCompanionBattleAnimation();
  }

  private playHeroDeathAnimation(): void {
    if (!this.heroDeathAnimKey) return;
    this.heroSprite.play(this.heroDeathAnimKey, true);
  }

  private playCompanionBattleAnimation(): void {
    this.companionSprites.forEach((sprite, idx) => {
      const key = this.companionBattleAnimKeys[idx];
      if (!key) return;
      this.playLoopAnimationIfNeeded(sprite, key);
    });
  }
